...
local_cell, dof_pos = compute_dofmap_pos(V)
...
proc_to_dof = np.zeros(num_owned_dofs, dtype=np.int32)
...
io = adios.DeclareIO("FunctionReader")
io.SetEngine("HDF5")
...
assert len(incoming_global_dofs) == num_owned_dofs
...
read_function_from_legacy_h5(MPI.COMM_WORLD, path, u2)
assert np.allclose(u.x.array, u2.x.array)
