...
def test_legacy_mesh():
    ...
    volume = mesh.comm.allreduce(dolfinx.fem.assemble_scalar(
        dolfinx.fem.form(1*ufl.dx(domain=mesh))), op=MPI.SUM)
    surface = mesh.comm.allreduce(dolfinx.fem.assemble_scalar(
        dolfinx.fem.form(1*ufl.ds(domain=mesh))), op=MPI.SUM)
...
def test_legacy_function():
    ...
    problem = dolfinx.fem.petsc.LinearProblem(a, L, [], uh, petsc_options={"ksp_type": "preonly",
                                                                           "pc_type": "lu"})
    problem.solve()
